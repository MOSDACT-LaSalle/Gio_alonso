...
void setup () {
  size(900, 900);
  posx = random(width);  //controls the arc size with the position of the mouse in X
  posy = random(height); //controls the spacing between each arc with the position of the mouse in Y
  background(0, 0, 0);
} // canvas setup
...
void draw () {
  ...
  distY = posy; // defining the variables into the mouse position
  arquito = posx;

  noFill(); // we are telling the system that the arcs must not have fill
  stroke(255,1); // arc stroke color is set to a random grayscale value on each draw cycle, creating glitches.
